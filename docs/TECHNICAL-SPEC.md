...
更新：2026-09-09。P00～P04 已有實作，程式規則為 `2.0.0`：限時手牌、留牌／換牌、決策窗口與五關路線契約已交付（[P04 報告](phase-reports/P04.md)），但只有第 1 關的內容通過驗收。五關戰役與分段結局的待實作契約見 [P04 改版計畫](P04-REVISION-PLAN.md)。不可把第 1 關的驗證當成五關完成。實際狀態以[進度表](DEVELOPMENT-STATUS.md)及階段報告為準。
...
| 介面 | 用途 |
|---|---|
| `GET /api/v1/levels` | 規劃五關、解鎖、可用情境摘要；目前仍是舊版三個代表情境 |
...
9. 驗證正式 URL、五關配置、來源更新、逐關勝敗、主線／進階終幕、續玩、SQLite 備份還原；保留一次排程成功與一次失敗備援證據。

## 8. 測試與效能門檻

- PHP 測試：資料 adapter、交易去重、版本衝突、擁有者隔離、規則、重播、複盤，採固定 fixture，不把真實上游當 CI 唯一依賴。
- Vue 測試：手牌／留牌／換牌、計時與逾時、事件演出、跳過／靜音／重新整理、回應重送；瀏覽器端到端測試走第 1、3、5 關、逐關勝敗與主線／進階兩種終幕。
