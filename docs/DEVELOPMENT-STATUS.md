...
| P06 正式視聽 | 未開始 | 全部必要素材、來源與演出；清單可由 `config/game.php` 的 15 張牌、5 個場景與 2 種終幕取得 |
...
|---|---|
| 本次指派階段 | P05 五關戰役 |
| 執行者及日期 | Claude Opus 5，2026-09-17 |
| 基準／交付 commit 或未提交變更 | 基準 `69c452a`；交付 `eb1d841`、`0dcd81e` 及本次文件與前端 commit |
| 階段報告相對路徑 | `docs/phase-reports/P05.md`、`docs/BALANCE.md` |
| 已通過項目 | `php artisan test` 164 通過、`npm run probe:test` 39 通過、`npx vue-tsc --noEmit` 無誤、`npm run build`、`vendor/bin/pint --dirty`、`game:simulate --seeds=100` 五關、桌機瀏覽器檢查（大廳、簡報、獎勵頁、主線終幕） |
| 尚未驗證／受阻原因 | 3.0.0 真人試玩未做（P04 遺留的試玩、錄影、手機實機也仍未補）；獎勵後的牌組未單獨掃 seed；混合資料情境未掃；獎勵頁與終幕頁未看手機 viewport；素材仍沿用第 1 關 |
| 下一位執行者第一步 | 依 P06 派工補五門禁術、五個場景與 15 張卡面素材；素材換上去不需要改規則。另需安排 3.0.0 的真人試玩 |
