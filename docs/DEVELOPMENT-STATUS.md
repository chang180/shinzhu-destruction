...
更新：2026-09-09。狀態只能使用 `未開始`、`進行中`、`待驗收`、`完成`、`受阻`。

目前已完成 P00 本機基線、P01 Laravel + Vue + SQLite + Boost 基礎、P02 三個部會資料 adapter 與快照備援、P03 伺服器權威戰鬥引擎與平衡鎖版，以及 P04 首關限時手牌改版（待驗收）。現行範圍是三關主線＋兩關進階，其中只有第 1 關內容已交付；詳見 [P04 改版計畫](P04-REVISION-PLAN.md) 與 [P04 報告](phase-reports/P04.md)。
...
| P04 首關限時手牌改版 | 待驗收 | [P04 報告](phase-reports/P04.md)：五張手牌、留牌／換牌、30 秒窗口與不限時練習、卡牌主畫面、五關路線契約已實作，`rules_version` 升至 `2.0.0`，145 個後端測試通過；**再次真人試玩、戰鬥畫面截圖與手機實機未做** |
| P05 五關戰役 | 未開始 | 第 2～5 關內容與平衡（目前 `available = false`）、順序解鎖、牌組獎勵、第 3 關收手與進階終幕 |
...
- `rules_version` 鎖在 `2.0.0`（`config/game.php`）；數值與公式以 [BALANCE.md](BALANCE.md) 為準，改動必須升版否則舊局重播會對不上。1.0.0 的舊局只能讀取與重播。
- 戰鬥引擎在 `app/Domain/Game/`。城市行為仍完全由關卡預告表決定，**沒有亂數**；2.0.0 唯一的亂數是牌序，而牌序完全由 `(seed, 第幾次洗牌)` 決定並保存在局面裡，所以相同輸入仍必得相同事件與結局。
- 一個回合分成 `reveal`（揭牌、開 30 秒窗口）與 `play`／`timeout`（結束回合）；`swap` 在窗口內可用一次。時鐘只有一個，在 `RunService`——引擎沒有時間概念，所以重播不受重播當下的時間影響。
...
- 五關固定在 `config/game.php`。`available` 是「內容做完了沒有」，和玩家解鎖無關；P04 只交付 `empty-cup`，其餘四關回 409 `level_unavailable`。
...
| 項目 | 內容 |
|---|---|
| 本次指派階段 | P04 首關限時手牌改版實作 |
| 執行者及日期 | Claude Opus 5，2026-09-09 |
| 基準／交付 commit 或未提交變更 | 基準 `ea5174b`；本輪實作尚未提交 |
| 階段報告相對路徑 | `docs/phase-reports/P04.md`、`docs/P04-REVISION-PLAN.md` |
| 已通過項目 | `php artisan test` 145 通過、`npm run probe:test` 28 通過、`npx vue-tsc --noEmit` 無誤、`npm run build`、`vendor/bin/pint --dirty`、`game:simulate` 100 seed、端到端 HTTP 煙霧測試、大廳瀏覽器渲染 |
| 尚未驗證／受阻原因 | 再次真人試玩未執行；卡牌戰鬥畫面沒有瀏覽器截圖（使用者中止瀏覽器操作）；手機未上實機；第 2～5 關內容與平衡未做；時長目標未量測 |
| 下一位執行者第一步 | 先補卡牌戰鬥畫面的瀏覽器與手機證據，再安排第二次真人試玩；這兩項補齊前 P04 不能標完成 |
