...
- Laravel is the only authority for future battle rules. The current campaign plan is three main levels plus two optional advanced levels (docs/P04-REVISION-PLAN.md), replacing the thirteen-level scope. Thirteen townships remain data coverage, not a level count. The player is a villain student; destroying the fictional city is the player's victory.
