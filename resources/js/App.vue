...
import { api, ApiError, PendingAction, elements, elementNames, skillName, unavailableText, eventText, cueImage, cueDuration, visibleEvents } from './game';
...
const pending = new PendingAction(sessionStorage);
...
const highlights = computed(() => allEvents.value.filter(e => ['impact', 'interrupt', 'city_repair', 'breach_opened'].includes(e.type)));
...
async function start(retry = false): Promise<void> {
    ...
    try {
        ...
        for (const name of ['city', 'apostle', 'water', 'heat', 'land']) { const image = new Image(); image.src = asset(name); }
    } catch (e) { handleError(e); } finally { busy.value = false; }
}
...
async function present(events: BattleEvent[]): Promise<void> {
    ...
    } finally { currentEvent.value = undefined; playing.value = false; stopWait = undefined; audio.stop(); }
}
...
function onKey(event: KeyboardEvent): void { if (event.key === 'Escape' && playing.value) skip(); }
...
<template>
    <div class="app-shell" :class="{ 'low-motion': reduced }">
        ...
        <div v-if="uncertain" class="message" role="status">施招結果待確認。<button :disabled="busy || playing" @click="submit(true)">重試原行動</button></div>
        ...
        <main>
            <template v-if="page === 'lobby'">
                <section class="hero">
                    ...
                    <div class="hero-copy"><p class="eyebrow">第一堂課 · 讓完美計畫，出一點差錯。</p><h1 tabindex="-1">超認真<br>毀滅新竹<span>計畫。</span></h1><p class="lead">城市有它的修復計畫。<br>而你，是計畫之外的那一筆。</p><button class="primary" :disabled="locked || !level" @click="start()">簽下入學計畫 <span>↗</span></button><p class="small">單人策略試煉 · 每局 {{ level?.max_turns ?? 10 }} 回合 · 預設靜音</p></div>
                    <span class="hero-caption">虛構城市演習 / 非真實災害預測</span>
                </section>
                <section class="lobby-bottom"><div><p class="eyebrow">CHAPTER 01</p><h2 tabindex="-1">空杯使徒 · {{ level?.name }}</h2><p>學會讀預告、留招、抓住破綻。<br>把城市核心韌性降至零，就是你的勝利。</p><p class="small">第 2～13 關仍在製作中。此版本開放完整第 1 關。</p></div><div class="save-list"><p class="eyebrow">你的試煉紀錄 · 本瀏覽器</p><p v-if="!savedRuns.length">還沒有紀錄。從第一次大膽的決策開始。</p><button v-for="saved in savedRuns" :key="saved.run_id" :disabled="locked" @click="openRun(saved.run_id)"><span>{{ saved.outcome === 'in_progress' ? '繼續試煉' : saved.outcome === 'player_victory' ? '毀滅成功・查看戰報' : '城市守住・查看戰報' }}</span><small>第 {{ saved.turn }} 回合 ↗</small></button><p class="small">匿名紀錄依賴本瀏覽器 Cookie；清除後無法找回。</p></div></section>
            </template>
            <template v-else-if="run && state">
                <section v-if="page === 'briefing'" class="briefing content-width">
                    <div class="briefing-art"><img :src="asset('apostle')" alt="空杯使徒：裂開的懸空陶器，城市燈點透過中空輪廓可見"><span>APOSTLE 01 / 空杯</span></div>
                    <div><p class="eyebrow">作戰簡報 · {{ level?.name }}</p><h1 tabindex="-1">別讓城市<br>把裂縫補回去。</h1><p class="lead">你有 {{ state.max_turns }} 回合。核心歸零便通關；回合用盡而城市仍站著，這次試煉就結束。</p><ol class="lesson"><li><b>讀預告。</b>每次施招後城市回應。第 3、6、9 回合會修復核心。</li><li><b>留擾序。</b>同系擾序可取消可打斷的預告。第 3 回合使用後，第 7 回合才能再用。</li><li><b>抓破綻。</b>破陣削弱防線，換系累積連攜。首次打斷還會獲得空杯使徒的惡意返還。</li></ol><button class="primary" :disabled="locked" @click="enterBattle">明白了，開始試煉 ↗</button><p class="small">本局情境已凍結。回學院後可繼續，不用一次打完。</p></div>
                </section>
                <section v-if="page === 'battle'" class="battle content-width">
                    <div class="battle-heading"><div><p class="eyebrow">CHAPTER 01 / {{ level?.name }}</p><h1 tabindex="-1">空杯的第一堂課</h1></div><div class="turn"><strong>{{ state.turn.toString().padStart(2, '0') }}</strong><span>/ {{ state.max_turns }} 回合</span></div></div>
                    <div class="battle-layout"><div class="battle-field">
                        ...
                        <div class="professor"><img :src="asset('professor')" alt="教授扶著厚計畫書給予提示"><div><small>教授的旁註</small><p>{{ hint }}</p></div></div>
                ...
                <section v-if="page === 'report'" class="report content-width" :class="{ victory: run.outcome === 'player_victory' }"><img :src="asset(run.outcome === 'player_victory' ? 'apostle' : 'city')" :alt="run.outcome === 'player_victory' ? '空杯使徒見證學員通關' : '守住的虛構城市'">
                    <div><p class="eyebrow">ACADEMY FIELD REPORT / 第一關戰報</p><h1 tabindex="-1">{{ run.outcome === 'player_victory' ? '毀滅成功。' : '城市守住了。' }}</h1><p class="lead">{{ run.outcome === 'player_victory' ? '學院認可你的決策。空杯，已留下裂痕。' : '本次試煉結束。把城市的回應，變成下一次的計畫。' }}</p><p>第 {{ state.turn }} 回合 · 核心剩餘 {{ state.core_resilience }} · 共 {{ run.history.length }} 次行動</p><p>{{ run.outcome === 'player_victory' ? '當防守只忙著修補，裂縫便有機會連成一線。以下是本局實際造成的影響。' : '城市仍保有核心韌性。查看修復與命中紀錄，再決定下一局要保留哪次擾序。' }}</p><div class="report-actions"><button class="primary" :disabled="locked || !run.compatible" @click="start(true)">同情境再試一次 ↗</button><button :disabled="locked" @click="replay">重播本局演出</button><button :disabled="locked" @click="loadLobby">回學院</button></div></div>
                </section>
                <section v-if="page === 'report'" class="content-width"><h2>關鍵回合 · 依實際紀錄</h2><ol class="highlights"><li v-for="(event, index) in highlights" :key="index"><b>第 {{ event.turn }} 回合</b><span>{{ eventText(event) }}</span></li></ol></section>
